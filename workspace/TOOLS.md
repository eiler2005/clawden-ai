...
Признаки: содержит `?`, начинается с «что», «как», «почему», «расскажи», «кто», «когда», «где», «объясни».
...
3. Вызвать `wiki_ingest({source_type: "text", source: "[извлечённый markdown]"})`
4. Ответить кратко: `✅ Сохранено: [title] → [domain]`
...
- При промоуте вызвать `wiki_ingest` и оформить структурированную запись
